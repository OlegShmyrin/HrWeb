    ...
    protected void processRequest (HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        if(answer==1){
            try {
                ...
                s.setStudentId(0);
                s.setDateOfBirth(new java.util.Date());
                ...
                sForm.initFromStudents(s);
                sForm.setGroups(groups);
                req.setAttribute("student",sForm);
                getServletContext().getRequestDispatcher("/StudentsFrame.jsp").forward(req,resp);
                return;
            }catch (SQLException sql_e){
                throw new IOException(sql_e.getMessage());
            }
        ...
        if(answer==2){
            try {
                if(req.getParameter("studentId")!=null){
                    int stId=Integer.parseInt(req.getParameter("stydentId"));
                    Student s = ManagementSystem.getInstance().getStudentById(stId);
                    Collection groups = ManagementSystem.getInstance().getGroups();
                    StudentForm studentForm = new StudentForm();
                    studentForm.initFromStudents(s);
                    studentForm.setGroups(groups);
        ...
        if(answer==3){
            String newGs=req.getParameter("groupId");
            String newYs = req.getParameter("year");
            try {
                Group g = new Group();
